...
action :set do
  if node['os_version'] >= '6.2'
    if exists_2012?
      Chef::Log.info('\"#{new_resource.name}\" found - attempting to set the
                     dns server.')
      new_resource.updated_by_last_action(true)
      if set_2012?
        Chef::Log.info('\"#{new_resource.server_addresses}\" is already set as
                       a DNS Server - nothing to do.')
        ...
        Chef::Log.info('\"#{new_resource.server_addresses}\" has been set as
                       the DNS Server.')
        new_resource.updated_by_last_action(true)
      end
    else
...
def exists_2008?
  check = Mixlib::ShellOut.new('powershell.exe -command netsh interface
                               #{new_resource.address_family} show interfaces
                               ').run_command
...
def exists_2012?
  check = Mixlib::ShellOut.new('powershell.exe -command
                               ...
                               \"#{new_resource.name}\" -AddressFamily
                               #{new_resource.address_family} -ErrorAction
                               SilentlyContinue').run_command
  !check.stdout.match('ObjectNotFound')
end

def set_2008?
  check = Mixlib::ShellOut.new('powershell.exe -command netsh interface
                               #{new_resource.address_family} show dns name=
                               \"#{new_resource.name}\"').run_command
...
def set_2012?
  check = Mixlib::ShellOut.new('powershell.exe -command
                               Get-DnsClientServerAddress -InterfaceAlias
                               \"#{new_resource.name}\" -AddressFamily
                               #{new_resource.address_family}').run_command 
  check.stdout.match(new_resource.server_addresses)
end
